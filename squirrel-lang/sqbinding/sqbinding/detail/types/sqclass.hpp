...
namespace sqbinding { namespace detail {
    class Class: public std::enable_shared_from_this<Class> {
        ...
        public:
            ...
            bool closed;
        public:
            Class (::SQClass* pClass, VM vm, bool closed = true): holder(std::make_shared<Holder>(pClass, vm)), closed(closed) {};
        public:
            void close() {
                closed = true;
            }
        ...
        public:
            template<typename TClass, typename P>
            void defProperty(std::string property, P TClass::* pm) {
                auto fget = detail::NativeClosure<P(TClass *)>::template Create<detail::cpp_function<1>>(
                    [=](TClass *self) -> P
                    {
                        return self->*pm;
                    },
                    holder->GetVM(), detail::cpp_function<1>::caller);
...
namespace sqbinding {namespace detail {
    ...
    public:
        template <class C>
        std::shared_ptr<Class> find_class_object() {
        ...
        template <class C>
        void register_class(std::shared_ptr<Class> clazz) {
