...
template <int paramsbase> class dynamic_args_function {
  public:
    struct Holder {
  ...
  public:
    ...
    dynamic_args_function(Return (Class::*f)(py::list) const) {
        initialize<true>(std::function([f](const Class *c, py::list args) -> Return { return (c->*f)(args); }),
    ...
    template <class Func, typename = std::enable_if_t<detail::function_traits<std::remove_reference_t<Func>>::value ==
                                                      detail::CppFuntionType::LambdaLike>>
  ...
  public:
    ...
    template <bool functor, class Func, typename Return> void initialize(Func &&f, Return (*signature)(py::list)) {
        struct capture {
  ...
  public:
    ...
    std::function<SQInteger(HSQUIRRELVM)> build_caller(
        Func &&f, typename std::enable_if_t<std::is_void_v<Return>, Return> * = nullptr) {
    ...
    std::function<SQInteger(HSQUIRRELVM)> build_caller(
        ...
        return std::function([f, this](HSQUIRRELVM vm) -> SQInteger {
            ...
            detail::generic_stack_push<Return>(vm, std::forward<Return>(v));
            return 1;
        });
    }

  public:
    template <typename Return> Return operator()(py::list args) {
        if (holder->functor) {
