...
PyValue sqbinding::python::pyobject_topyvalue(py::object object) {
    __try_cast_pyobject_topyvalue(v, object, py::none) if (py::isinstance<py::int_>(object)) {
...
#define __try_cast_userdata(object, typetag, cpp_type)                                                                 \
    if (_userdata(object)->_typetag == (void *)typetag) {                                                              \
        ...
        std::cout << "[TRACING] cast SQObjectPtr to " << typeid(decltype(holder->GetInstance()._val)).name()           \
                  << std::endl;                                                                                        \
