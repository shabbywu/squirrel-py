...
SQRESULT sq_newmember(HSQUIRRELVM v,SQInteger idx,SQBool bstatic)
{
    ...
    if(sq_type(v->GetUp(-3)) == OT_NULL) return sq_throwerror(v, _SC("null key"));
...
SQRESULT _getmemberbyhandle(HSQUIRRELVM v,SQObjectPtr &self,const HSQMEMBERHANDLE *handle,SQObjectPtr *&val)
{
    switch(sq_type(self)) {
        ...
        case OT_CLASS: {
            ...
            break;
        default:
            return sq_throwerror(v,_SC("wrong sq_type(expected class or instance)"));
    }
    return SQ_OK;
}
